...
class ChatGPT(AbstractLLM):
    ...
    def _load_api_keys(self):
        ...
        # 可能的配置文件路径
        possible_paths = [
            os.path.join(os.path.dirname(__file__), '..', 'api_keys.py'),  # 项目根目录
            os.path.join(os.path.dirname(__file__), 'api_keys.py'),  # core目录
        ]
        ...
        if not config_file:
            raise FileNotFoundError(
                ...
                "   请按以下步骤配置:\n"
                "   1. 复制示例文件: cp api_keys.example.py api_keys.py\n"
                "   2. 编辑 api_keys.py，填入你的真实API密钥\n"
                "   3. ⚠️ 不要将 api_keys.py 提交到Git仓库"
            )
